...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        ...
        elArray = new LEGListaConPI[nuevaCapacidad];
        
        //Inicializamos cada posicion del array con ListasConPI
        for(int i = 0; i < elArray.length; i++) {
            elArray[i] = new LEGListaConPI<EntradaHash<C,V>>();
        }
        
        //Copiamos todas las entradas de aux a elArray
        for(int i = 0; i < aux.length; i++) {
            ListaConPI<EntradaHash<C,V>> lista = aux[i];
            for(lista.inicio(); !lista.esFin(); lista.siguiente()) {
                EntradaHash<C,V> entrada = lista.recuperar();
                elArray[indiceHash(entrada.clave)].insertar(entrada);
            }
        }
        numRH++;
    }
    ...
    public final double costeMLocalizar() {
        /* COMPLETAR */
        double numColisiones = 0;
        for(int i = 0; i < elArray.length; i++) {
            numColisiones += (elArray[i].talla() * (elArray[i].talla() - 1) / 2);
        }
        return numColisiones / this.talla();
    }
